...
@csrf_exempt
async def web_scrapping(request):
    if request.method == 'POST':
        try:
            ...
            async with async_playwright() as p:
                ...
                await page.route("**/*.{mp4,webm,ogg,avi,mov,gif,jpg,jpeg,png,bmp,tiff}", lambda route: route.abort())
                ...
                try:
                    ...
                    await browser.close()
                    return JsonResponse({"error": f"Failed to load page: {str(e)}"}, status=400)
                ...
                try:
                    ...
                    for link in links:
                        ...
                        if href and ('about' in href.lower() or 'about-us' in href.lower()):
                            about_us_link = link
                            break

                    if about_us_link:
                        about_us_url = await about_us_link.get_attribute('href')
                        if not about_us_url.startswith('http'):
                            about_us_url = urljoin(url, about_us_url)
                        await page.goto(about_us_url, timeout=100000)
                        await page.wait_for_load_state('domcontentloaded', timeout=100000)
                        print(f"Now on the About Us page: {page.url}")
                except Exception as e:
                    print(f"Error finding About Us page: {str(e)}")

                # Get final page content
                final_url = page.url
                await browser.close()

            # Scrape content with requests
            headers = {
